...
using namespace RooFit;
 ...
 { 
   ...
   //   std::cout <<"t " <<t<<" alpha 1 "<<alpha1<<" alpha 2 "<<alpha2<<" x "<<x<<" mean "<<mean<< std::endl;
   double val = -99.;
...
//      val = A2*gbrmath::fast_pow(B2+t,-n2);
     //  }else{
     //std::cout << "ERROR evaluating range: t " <<t<<" x "<<x<<" mean "<<mean<<" width "<<width<<" alpha1 "<<alpha1<<" alpha2 "<<alpha2<< std::endl;
   }
   if (!std::isnormal(val)) {
